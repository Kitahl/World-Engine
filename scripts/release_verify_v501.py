#!/usr/bin/env python3
"""Generate independent World Engine 5.0.1 release audits."""
...
EXPECTED_FEATURES = {
    **inherited.EXPECTED_FEATURES,
    "procedural_desktop_companion": "5.0.0",
    ...
    "qualification_hardening": RELEASE,
}
...
def source_audit() -> dict[str, Any]:
    ...
    instruction_path = ROOT / "CUSTOM_GPT_INSTRUCTIONS_V501.txt"
...
def main() -> int:
    ...
    for name, result in results.items():
        _write(args.output_dir / f"WORLD_ENGINE_V501_{name.upper()}_AUDIT.json", result)
    summary = {
        "release": RELEASE,
        "audits": {name: bool(result["passed"]) for name, result in results.items()},
    }
    summary["passed"] = all(summary["audits"].values())
    _write(args.output_dir / "WORLD_ENGINE_V501_RELEASE_AUDIT.json", summary)
