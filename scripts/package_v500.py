#!/usr/bin/env python3
"""Build and independently verify the full World Engine 5.0.0 Windows archive."""
...
RELEASE = "5.0.0"
PACKAGE_ROOT = "world_engine_v5_0_0_RUNTIME_CONVERGENCE_WINDOWS_FULL"
PACKAGE_NAME = PACKAGE_ROOT + ".zip"
HANDOFF_NAME = "WORLD_ENGINE_V500_HANDOFF.json"
CRITICAL_FILES = (
    ...
    "CUSTOM_GPT_INSTRUCTIONS_V500.txt",
    "GPT_INSTRUCTIONS.md",
    "README.md",
    ...
    "scripts/release_verify_v500.py",
    "scripts/package_v500.py",
...
def verify_tree(tree: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="world-engine-v500-audits-") as temporary:
        ...
        narrative_result = inherited.run(
            [
                ...
                "scripts/narrative_release_audit.py",
                "--output",
                str(audit_dir / "WORLD_ENGINE_V500_NARRATIVE_AUDIT.json"),
            ],
            cwd=tree,
            check=False,
            timeout=300,
...
def verify_extracted(zip_path: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="world-engine-v500-extracted-") as temporary:
...
def main() -> int:
    ...
    final_hashes = {
        name: inherited.sha256_file(ROOT / name) for name in CRITICAL_FILES
    }
    stable = (
