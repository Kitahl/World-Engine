#!/usr/bin/env python3
"""Generate independent World Engine 5.1.0 release audits.

Inherits the 5.0.1 audit surface unchanged and re-pins the identities that
5.1.0 actually moves: the release string, the active instruction file, the
desktop projection contract, and the desktop companion feature version.
Everything else - schema 24, exactly five public Actions, PBEM 2.2,
NRP/NQR 1.2, WEGEN-2.0 - is asserted to be UNCHANGED.
"""
...
EXPECTED_FEATURES = {
    **inherited.EXPECTED_FEATURES,
    "procedural_desktop_companion": "5.1.0",
    "canonical_mechanism_contract": "5.0.0",
    "event_incident_runtime": "5.0.0",
    "politics_commitment_runtime": "5.0.0",
    "actor_agency_runtime": "5.0.0",
    "quest_graph_runtime": "5.0.0",
    "qualification_hardening": "5.0.1",
}
...
def source_audit() -> dict[str, Any]:
    ...
    instruction_path = ROOT / "CUSTOM_GPT_INSTRUCTIONS_V510.txt"
...
def runtime_audit() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="world-engine-v501-") as temporary:
        ...
        public_types = {
            row["event_type"]
            for row in engine.get_world_context("release")["recent_events"]
        }
        ...
        checks = {
            ...
            "generated_runtime_rows": all(
                value > 0 for value in runtime_counts.values()
            ),
            "desktop_projection": projection["schema"] == "WE-DESKTOP-5.1.0",
            # 5.1.0 additive projection fields must be present and typed.
            "desktop_projection_sequence": isinstance(
                projection.get("projection_sequence"), int
            ),
            "desktop_terrain_seed": isinstance(projection.get("terrain_seed"), int)
            and projection["terrain_seed"] >= 0,
            "desktop_notification_tier": str(
                (projection.get("notification_summary") or {}).get("tier")
            )
            in {"normal", "warning", "critical"},
            "desktop_local_domains": all(
                key in projection for key in ("journal", "agency", "politics", "quests")
            )
...
def main() -> int:
    ...
    for name, result in results.items():
        _write(args.output_dir / f"WORLD_ENGINE_V510_{name.upper()}_AUDIT.json", result)
    summary = {
        "release": RELEASE,
        "audits": {name: bool(result["passed"]) for name, result in results.items()},
    }
    summary["passed"] = all(summary["audits"].values())
    _write(args.output_dir / "WORLD_ENGINE_V510_RELEASE_AUDIT.json", summary)
