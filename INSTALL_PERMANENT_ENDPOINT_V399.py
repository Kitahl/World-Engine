...
def ensure_launcher_config(data: Path) -> tuple[Path,str,bool]:
    ...
    if len(key)<24:
        key=secrets.token_urlsafe(32); cfg["api_key"]=key; cfg["created_by"]="World Engine 4.7.0 permanent endpoint installer"; atomic_json(p,cfg); created=True
...
def main() -> int:
    ...
    print(f"[4.7.0] Local World Engine PASS; API-key fingerprint {api_key_fingerprint(key)}")
    ...
    print(f"\n[4.7.0] PERMANENT ENDPOINT PASS: {result['public_url']}")
    print("[4.7.0] No Administrator rights were used by ngrok user mode.")
    return 0
