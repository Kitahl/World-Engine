...
def authenticated_probe(base_url: str, api_key: str, timeout: float = 5.0) -> tuple[bool, int | None, str]:
    ...
    req = urllib.request.Request(url, headers={"Authorization": f"Bearer {api_key}", "User-Agent": "WorldEngineLauncher/5.0.0"})
    try:
...
class Launcher(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("World Engine v5.0.0 — Action Connection Diagnostics")
    ...
    def _build_ui(self) -> None:
        pad = {"padx": 10, "pady": 6}
        title = ttk.Label(self, text="World Engine v5.0.0", font=("Segoe UI", 18, "bold"))
    ...
    def _ensure_cloudflared(self) -> Path:
        ...
        req = urllib.request.Request(CLOUDFLARED_URL, headers={"User-Agent": "WorldEngineLauncher/5.0.0"})
